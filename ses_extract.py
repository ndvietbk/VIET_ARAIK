...
def parse_args():
    ...
    parser.add_argument('-t', '--maxtime', default=0.001, type=float,
                        help='largest session time allowed')
    parser.add_argument('-n', '--ipnum', default=1, type=int,
...
def extract_session(input_file, output_file, protocol, maxtime, ipnum):
    ...
    for i in bar(range(ipnum)):
        ...
        if ind != []:
            ...
            ssize1 = np.asarray(np.split(ssize2, ind[0]))
            ssize = [np.sum(a) for a in ssize1]
            if ssize[0] == 0:
                ssize[0] = ssize2[0]
            ncon1 = np.asarray(np.split(ncon2, ind[0]))
        ...
        if (i == 0):
            out = pd.concat([stime, sip, ncons, ssize], axis=1)
        else:
            out1 = pd.concat([stime, sip, ncons, ssize], axis=1)
            out = out.append(out1)
