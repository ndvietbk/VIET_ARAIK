...
fdate <- c(20150416, 20150417, 20150418, 20150419)
...
for (i in 1:length(fdate)) {
  ...
  svar <- beck_sim(length(fdate)*18750, lam2)
  svar <- svar/mean(svar)
  ...
  lam3 <- numeric(n2)
  lam3[pi] <- lam2
  svar2 <- beck_sim(length(fdate)*18750, lam3)
  svar2 <- svar2/mean(svar2)
  ps2 <- density(svar2, from=0.5, to=xmax, n=100)
  outs2 <- data.frame(xs2=ps2$x, ps2=ps2$y)
  ...
  pl <- pl + geom_point(aes(x = x, y = p), shape=1, color = "red", size = 3)+
            ...
            geom_point(aes(x = xs2, y = ps2), shape=3, color = "brown", size=3)+
